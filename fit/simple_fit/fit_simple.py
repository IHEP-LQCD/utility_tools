...
# data for 2pt of glueball
def make_data_all(corre, tmin, tmax):

    x = {}
    x["tg"] = np.arange(tmin[0], tmax[0] + 1)

    data = {}
    data["GG"] = corre[0][:, tmin[0]: tmax[0] + 1]
    print("GG: xmin = {}, xmax = {}".format(tmin[0], tmax[0]))
...
def do_fit(options, paramlabel, icorre, data):
    ...
    for tmin in range(rmin[icorre], rmin[icorre] + 8):
        ...
        fit = lsqfit.nonlinear_fit(
            data=(x, y), fcn=fcn_all_twostate, prior=prior, p0=None
        )
        print(fit)
...
def main():
    ...
    labels = [r"$m_{GG}^{eff}(t)a_t$"]

    data = read_data(dim, infile)
    avegg = np.average(data[0], axis=0)
    data /= avegg[0]

    paramlabel = ["Mg1", "Mg2"]
    xlabels = [r"$t_{gg_{min}}/a_t$"]
    ylabels = [r"$m_{g_1}a_t$", r"$m_{g_2}a_t$"]
    ...
    plot_effem_and_fit(
        options, 0, labels[0], data[0], "GG", fcn_all_twostate, choose_fit
    )
